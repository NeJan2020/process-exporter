...
func NewGrouper(namer common.MatchNamer, trackChildren, trackThreads, alwaysRecheck, minimal, debug, removeEmptyGroups bool) *Grouper {
	g := Grouper{
		groupAccum:        make(map[string]Counts),
		threadAccum:       make(map[string]map[string]Threads),
		tracker:           NewTracker(namer, trackChildren, alwaysRecheck, minimal, debug),
		debug:             debug,
...
func (g *Grouper) groups(tracked []Update) GroupByName {
	...
	// Add groups observed in the past but no longer running, unless configured
	// to remove them. Also clear their accumulated counters when removing them
	// so that a later group with the same name starts with fresh counters.
