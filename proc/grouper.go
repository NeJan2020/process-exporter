...
func NewGrouper(namer common.MatchNamer, trackChildren, trackThreads, minimalMetrics, recheck bool, recheckTimeLimit time.Duration, debug bool, removeEmptyGroups bool) *Grouper {
	g := Grouper{
		groupAccum:        make(map[string]Counts),
		threadAccum:       make(map[string]map[string]Threads),
		tracker:           NewTracker(namer, trackChildren, minimalMetrics, recheck, recheckTimeLimit, debug),
		debug:             debug,
